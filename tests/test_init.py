...
async def test_async_setup_registers_cards(hass: HomeAssistant) -> None:
    ...
    with patch("homeassistant.components.frontend.add_extra_js_url") as mock_add_js_url:
    ...
    assert [c.url_path for c in configs] == [
        "/adam_audio/adam-audio-card.js",
        "/adam_audio/adam-audio-backplate-card.js",
        "/adam_audio/adam-audio-backplate-card-alt.js",
    ]
    assert mock_add_js_url.call_count == 3
