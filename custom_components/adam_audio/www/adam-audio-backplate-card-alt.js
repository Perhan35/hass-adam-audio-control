 ...
 * Alternative to the custom Lovelace card replicating the physical backplate of ADAM Audio A-Series monitors.
 *
...
<svg viewBox="0 0 48 48" fill="none">
  ...
  class AdamAudioBackplateCard extends HTMLElement {
    ...
    setConfig(config) {
      if (!config.entities) throw new Error("adam-audio-backplate-card-alt: 'entities' is required.");
      this._config = config;
      this._rendered = false;
    }
  ...
  try {
    customElements.define("adam-audio-backplate-card-alt", AdamAudioBackplateCard);
  } catch (e) {
    if (!e.message.includes("already been used")) {
      console.error("Failed to register adam-audio-backplate-card-alt:", e);
    }
  }
  ...
  window.customCards.push({
    type: "adam-audio-backplate-card-alt",
    name: "ADAM Audio Backplate Card Alternative",
  ...
  console.info(
    `%c ADAM AUDIO BACKPLATE ALTERNATIVE %c v${BP_VERSION} `,
